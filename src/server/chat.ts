...
import type { AuthData, OpenAIChatRequest, OpenAIMessage } from "../shared/types.js";
import {
  buildPrompt,
  createStreamState,
  applyStreamEvent,
...
export function chatRouter(getClient: () => DeepSeekClient): Router {
  ...
  router.post("/v1/chat/completions", async (req: Request, res: Response) => {
    ...
    const thinkingEnabled = body.thinking ?? modelId === "deepseek-reasoner";
    const stream = body.stream ?? true;

    try {
      // Build a conversation key from first user message hash
      ...
      if (stream) {
        ...
        // Send final chunk
        const finalChunk = makeChunk(modelId, {}, "stop");
